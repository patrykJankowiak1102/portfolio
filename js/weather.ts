function autocomplete(inp: HTMLInputElement | null, arr: string[]) {
	if (!inp) {
	...
	inp.addEventListener("input", function(this: HTMLInputElement, e: Event) {
		...
		for (i = 0; i < arr.length; i++) {
			if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
				...
				b.addEventListener("click", function(this: HTMLDivElement, e: Event) {
					if (inp) {
	...
	inp.addEventListener("keydown", function(this: HTMLInputElement, e: KeyboardEvent) {
		let x: HTMLCollectionOf<HTMLDivElement> | undefined = document.getElementById(this.id + "autocomplete-list")?.getElementsByTagName("div");
		if (x && x.length > 0) {
			if (e.keyCode == 40) {
				currentFocus++;
				addActive(x);
	...
	function closeAllLists(elmnt?: HTMLElement) {
		let x = document.getElementsByClassName("autocomplete-items") as HTMLCollectionOf<HTMLElement>;
		for (let i = 0; i < x.length; i++) {
	...
	document.addEventListener("click", function(e: MouseEvent) {
		if (inp && e.target instanceof Node) {
...
autocomplete(document.getElementById("inputCountry") as HTMLInputElement, countries);